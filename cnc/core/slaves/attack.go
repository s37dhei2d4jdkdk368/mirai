...
func (cl *ClientList) worker() {
    for {
        ...
        case add := <-cl.addQueue:
            ...
            var existingUID int
            var existingBot *Bot
            for uid, bot := range cl.clients {
                if bot.conn.RemoteAddr().String() == add.conn.RemoteAddr().String() {
                    existingUID = uid
                    existingBot = bot
                    break
                }
            ...
            if existingBot != nil {
                // Duplicate found: close old connection and replace with new one
                existingBot.conn.Close()
                delete(cl.clients, existingUID)
                add.uid = existingUID
                cl.clients[existingUID] = add
                // Don't increment count since we're replacing, not adding
            } else {
                // No duplicate: normal add
                cl.totalCount <- 1
                cl.uid++
                add.uid = cl.uid
        ...
        case del := <-cl.delQueue:
            // Check if this bot is still the one in our map (not replaced)
            if existingBot, ok := cl.clients[del.uid]; ok {
                // Only decrement count if this is the same bot (same connection)
                if existingBot.conn.RemoteAddr().String() == del.conn.RemoteAddr().String() {
                    cl.totalCount <- -1
                    delete(cl.clients, del.uid)
                }
                // If this was a replaced bot, the count was never incremented,
                // so we don't need to decrement it
            } else {
                // Bot already removed (likely replaced), no need to decrement count
                delete(cl.clients, del.uid)
            }
